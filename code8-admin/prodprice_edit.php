...
{			
	...
	$hiddenprods = $_POST['hiddenprods'];
	
	for($i=0;$i<=$countcomb;$i++){	
		if($price[$i]!=""){
			if($hiddencombi[$i]!="" && $hiddenprods[$i]!=""){			
				$adminmenu_query = "UPDATE code8_prodprices SET prodid='$product', country='$country[$i]', price='$price[$i]' WHERE prodid=$id AND country=$hiddencombi[$i]";
...
?>
<?php
	$banner_que = "SELECT * from code8_prodprices where prodid=$id";
			...
			<div class="x_panel">
			  ...
			  <div class="x_content">
				...
				foreach($numcont_res as $numcont_result)
				{
					...
					$banner_queval = "SELECT * from code8_prodprices where country=$contryid AND prodid=$prodid";
					$database1 = new Database();
					$dbCon1 = $database1->getConnection();
					$stmt1 = $dbCon1->prepare($banner_queval);  
					$stmt1->execute();	
					$about_resval = $stmt1->fetch(PDO::FETCH_ASSOC);
					$prodval = $about_resval['price'];	
				?>
				<input type="hidden" name="hiddencombi[]" value="<?php echo $numcont_result['id']; ?>">
				<input type="hidden" name="hiddenprods[]" value="<?php echo $prodval; ?>">				
				  <div class="form-group">
					...
					<div class="col-md-2 col-sm-2 col-xs-12">
						<input type="text" name="price[]" id="<?php echo $numcont_result['currencycode']; ?>" required="required" value="<?php echo $prodval; ?>" class="form-control" >
					</div>
				  </div>
				<?php } ?>

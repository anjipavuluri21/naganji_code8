  ...
  $.fn.drilldownSelect = function(params) {
    ...
    var defaults = {
      ...
      onSelected: function(event) {
      	//alert($(event.target).data('id')); //id returns here
      }
    };
    ...
    function buildPathValue(path, dataToSearch) {
      ...
      if (path.length) {
        var pathCurrentId = path.shift();
		 console.log(pathCurrentId);
		//$('#hidcategid').val(pathCurrentId);
		
		
		  /* ****************Patch to inject parent id and child id or parentid only to hidden variable */			 
				$('#hidcategid').val(pathCurrentId);
				/********************************/
        ...
        if (!disabled) {
          ...
          childItem.on('click', function(event) {
            ...
            if (textHolder.html() == textHolder.attr('placeholder')) {
              ...
              textHolder.html('');
			//  alert(eventTarget.data('id'));
			  
			  /* ****************Patch to inject parent id to hidden variable */
			  var selectedcategid=$(event.target).data('id');
				$('#hidcategid').val(selectedcategid);
				/********************************/
            }

...
$(document).ready(function(){
	...
	$('.profile-mobile-link').click(function(){
		$('.my-account-right').toggleClass('opened');
		$('.profile-mobile-link').toggleClass('active');
	});
	...
	$('.size-heading').click(function(){
		console.log('.size-heading');
		$('.size-carousel').slideToggle();
		$('.size-alert').slideUp();
		sizeSliderOpen();
		sizeAlertClose();
	});
	...
	$('.color-radios li input').click(function() {
		if($(this).is(":checked")){
			$('.color-alert').slideUp();
		}
	});
...
function sizeSliderOpen(){
	//swipersize.update();
	createSwiper();
	product_size=$('.size-container .swiper-slide a').width();
	$('head').append('<style type="text/css">.size-container .swiper-slide a{height:'+product_size+'px}</style>');
}
function sizeAlertClose(){
	$('.size-alert').slideUp();
}
function sizeAlertOpen(){
...
$(document).mouseup(function(e)
{
	...
	var wsmenu1 = $('.wsmenu-submenu');
	var wsmenu2 = $('.wsmenu-click');
	if (!wsmenu1.is(e.target) && wsmenu1.has(e.target).length === 0) 
	{
		if (!wsmenu2.is(e.target) && wsmenu2.has(e.target).length === 0) 
		{			
			$('.wsmenu-submenu').slideUp();
			$('.wsmenu-click').removeClass('ws-activearrow');
		}
	}
	var profile1 = $('.my-account-right');
	var profile2 = $('.profile-mobile-link');
	if (!profile1.is(e.target) && profile1.has(e.target).length === 0) 
	{
		if (!profile2.is(e.target) && profile2.has(e.target).length === 0) 
		{			
			$('.my-account-right').removeClass('opened');
			$('.profile-mobile-link').removeClass('active');
		}
	}
	//$('.shoppingcart-popup .product-thumb .remove-item').removeClass('active');
		//$('.shoppingcart-popup .wishlist-remove').removeClass('opened');
...
var cart_page_pro_img;
var total_cart_page_pro_img;
var pro_cont_pro_img;
var favourite_product_wd;
var product_size;
var order_review_wd;
var order_review_ht;
var order_history_wd;
var order_history_ht;
var my_cart_sub_wd;
var my_cart_sub_ht,rect_width;
function addRemoveStyle(){
	...
	rect_width = $('.content-box').innerWidth();
	
	if(win_width>780){
		$('.box rect,.box svg').css({'height':(rect_width-17)});
		$('.box rect,.box svg').css({'width':(rect_width-17)});
	}
	else{
		$('.box rect,.box svg').css({'height':(rect_width-8)});
		$('.box rect,.box svg').css({'width':(rect_width-8)});		
	}
	product_holder_img_fav = $('.product-div li .product-holder-img').width();
	total_pro_ht = product_holder_img_fav*1.33;
	//$('.product-div li .product-holder-img,.product-div figure').css({'height':total_pro_ht});
	$('.favourite-scroll').css({'height':total_pro_ht});
	$('head').append('<style type="text/css">.product-div li .product-holder-img,.product-div li figure{height:'+total_pro_ht+'px}.product-holder-img img{width:auto;min-width:100%;height:100%;}</style>');
	
	/*favourite_product_wd=$('.favourite-product .product-holder-img').width()*1.15;
	$('head').append('<style type="text/css">.favourite-product .product-holder-img figure{height:'+favourite_product_wd+'px}</style>');*/
	
	product_holder_img = $('.product-box .product-holder-img').width();
	total_pro_ht_two = product_holder_img*1.33;
	//$('.product-box .product-holder-img,.product-box figure').css({'height':total_pro_ht_two});
	$('head').append('<style type="text/css">.product-box .product-holder-img,.product-box figure{height:'+total_pro_ht_two+'px}</style>');
	
	pro_cont_pro_img=$('.product-container .swiper-slide .product-img').width()*1.33;
	$('head').append('<style type="text/css">.product-container .swiper-slide .product-img{height:'+pro_cont_pro_img+'px}</style>');
	
	cart_page_pro_img = $('.product-thumb-sub .product-th-left').width();
	total_cart_page_pro_img = cart_page_pro_img*1.33;
	$('head').append('<style type="text/css">.product-thumb-sub .product-holder-img figure{height:'+total_cart_page_pro_img+'px}</style>');
	...
	//$('.cart-product .product-holder-img,.cart-product figure').css({'height':cart_product_ttl});
	$('head').append('<style type="text/css">.cart-product .product-holder-img,.cart-product figure{height:'+cart_product_ttl+'px}</style>');
	...
	$('head').append('<style type="text/css">.designer-box .designer-img{height:'+designer_product_ttl+'px}</style>');
	
	/*portfolio_img = $('.portfolio-box .portfolio-img').width();
	portfolio_ttl = portfolio_img*1.33;
	$('head').append('<style type="text/css">.portfolio-box .portfolio-img{height:'+portfolio_ttl+'px}</style>');*/
	
	my_account_fav_img = $('.my-account-fav .product-holder-img').width();
	my_account_fav_img_ttl = my_account_fav_img*1.33;
	//$('.my-account-fav .product-holder-img,.my-account-fav figure').css({'height':my_account_fav_img_ttl});
	$('head').append('<style type="text/css">.my-account-fav .product-holder-img,.my-account-fav figure{height:'+my_account_fav_img_ttl+'px}</style>');
	...
	$('head').append('<style type="text/css">.get-look-main-img{height:'+get_look_main_img_ttl+'px}</style>');
	order_review_wd=$('.order-review .product-thumb .product-holder-img').width();
	order_review_ht=order_review_wd*1.33;
	$('head').append('<style type="text/css">.order-review .product-thumb .product-holder-img figure{height:'+order_review_ht+'px}</style>');

	order_history_wd=$('.order-histotry-sub .product-thumb .product-holder-img').width();
	order_history_ht=order_history_wd*1.33;
	$('head').append('<style type="text/css">.order-histotry-sub .product-thumb .product-holder-img figure{height:'+order_history_ht+'px}</style>');
	
	my_cart_sub_wd=$('.my-cart-items-sub .product-thumb .product-holder-img').width();
	my_cart_sub_ht=my_cart_sub_wd*1.33;
	$('head').append('<style type="text/css">.my-cart-items-sub .product-thumb .product-holder-img figure{height:'+my_cart_sub_ht+'px}</style>');
...
function product_scroll(){
	...
	total_pro_ht = product_holder_img_fav*1.33;
	//$('.product-div li .product-holder-img,.product-div figure').css({'height':total_pro_ht});
	$('head').append('<style type="text/css">.product-div li .product-holder-img,.product-div li figure{height:'+total_pro_ht+'px}</style>');
	$('.favourite-scroll').css({'height':total_pro_ht});
}
...
$(window).load(function() {
	"use strict";	
	addRemoveStyle();
	//$('.addedToCartSuccessfully').trigger( "click" );
});

...
$currencycode = $countrydet['currencycode'];
	...
	$prodcount_que = "SELECT COUNT(*) as num_rows, cp.id as pid from code8_products cp LEFT JOIN code8_prodprices cpp ON cp.id=cpp.prodid LEFT JOIN code8_stocks cs ON cp.id=cs.prodid where cp.sale=1 AND cp.status=1 AND cpp.country='$countryid' $cond";
	...
	//get rows query		
	$remprodcount_que = "SELECT *, cp.id as pid from code8_products cp LEFT JOIN code8_prodprices cpp ON cp.id=cpp.prodid LEFT JOIN code8_stocks cs ON cp.id=cs.prodid where cp.sale=1 AND cp.status=1 AND cpp.country='$countryid' $cond LIMIT ".$showLimit;
	$database1 = new Database();
	$dbCon1 = $database1->getConnection();
